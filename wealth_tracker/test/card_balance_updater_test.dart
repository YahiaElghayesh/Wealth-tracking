...
void main() {
  ...
  test('prefers the balance the SMS itself stated over deriving one', () async {
    ...
    expect(updated.balanceUpdatedSource, 'sms');
    expect(card.currentAvailableBalance, 50000); // original row untouched
  });

  test(
    'a charge with no stated balance subtracts from what was last known',
    () async {
  ...
  test(
    'a payment/credit with no stated balance adds instead of subtracting',
    () async {
      await insertCard(lastFourDigits: '1234', currentAvailableBalance: 10000);
      final parsed = ParsedBankSms(
  ...
  test(
    'a charge with nothing known yet falls back to the card limit',
    () async {
  ...
  test(
    'a currency mismatch between the SMS and the card is not applied',
    () async {
  ...
  test(
    'a stated balance in the card\'s currency is applied even if the charge itself was a different currency',
    () async {
  ...
  test(
    'no last-4-digits on the SMS returns null (nothing to match against)',
    () async {
      await insertCard(lastFourDigits: '1234');
      final parsed = ParsedBankSms(
        vendor: 'Somewhere',
  ...
  test(
    'updates a card on a different profile than whichever one is passed around elsewhere',
    () async {
      ...
      await insertCard(
        lastFourDigits: '4912',
        currentAvailableBalance: 50000,
        profileId: 'family-member-profile',
      );
      final parsed = ParsedBankSms(
        vendor: 'Card refund',
        amount: 1025,
