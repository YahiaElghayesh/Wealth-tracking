...
Future<PendingSms?> takePendingSms() async {
  final result = await _channel.invokeMapMethod<String, dynamic>('takePendingSms');
  if (result == null) return null;
  return PendingSms(body: result['body'] as String, timestampMillis: (result['timestampMillis'] as num).toInt());
}
...
void listenForNewSms(void Function(PendingSms sms) onSms) {
  _channel.setMethodCallHandler((call) async {
    ...
    onSms(PendingSms(body: args['body'] as String, timestampMillis: (args['timestampMillis'] as num).toInt()));
  });
}
